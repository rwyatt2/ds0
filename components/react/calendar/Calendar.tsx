            ...
            <div ref={ref} className={cn('rounded-lg p-4', variant === 'bordered' ? 'border' : 'bg-card shadow-sm border', className)} {...props}>
                ...
                <div {...calendarProps}>
                    <div role="row" className="grid grid-cols-7 mb-1">
                        {WEEKDAYS.map(d => <span key={d} role="columnheader" className="text-center text-xs font-medium text-muted-foreground py-1">{d}</span>)}
                    </div>
                    {weeks.map((week, wi) => (
                        <div key={wi} role="row" className="grid grid-cols-7">
                            {week.map((day, di) => (
                                <span key={di} role="gridcell" aria-selected={day.isSelected} className="flex items-center justify-center">
